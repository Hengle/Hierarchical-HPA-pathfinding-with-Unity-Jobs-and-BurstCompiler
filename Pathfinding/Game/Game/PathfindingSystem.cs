...
public class PathfindingSystem : ComponentSystem
{
    ...
    [BurstCompile] private struct FindPathJob : IJobParallelFor
    {
        ...
        private NativeList<int2> FindConcretePath( NativeList<int3> abstractPath )
        {
            ...
            int2 endCluster = new int2(
                endNode.x / graphClusterLength ,
                endNode.y / graphClusterLength );
            ...
            int endClusterIndex = endCluster.x + endCluster.y * graphClusterLength;
            ...
            // find a low level path in the last cluster from the first intra-index to the start position
            ...
            while ( toIndex < lastIndex )
            {
                ...
                int2 clusterPosition = new int2(
                    fromPosition.x / graphClusterLength ,
                    fromPosition.y / graphClusterLength );
                int clusterIndex =
                    clusterPosition.x + clusterPosition.y * graphClusterLength;
                ...
                while ( notBlockedOrAtEnd )
                {
                    ...
                    if ( !LOSBetween2Clusters( start , end ) )
                    {
            ...
            int2 firstClusterPosition = new int2(
                toPosition.x / graphClusterLength ,
                toPosition.y / graphClusterLength );
            int firstClusterIndex = 
                firstClusterPosition.x + firstClusterPosition.y * graphClusterLength;

            if ( graphClustersDense[ firstClusterIndex ] != 0 )
            {
                int2 cachedPathKey =
                    graphEdgePaths[ abstractPath[ toIndex ].z ];
            ...
            {
                int index = abstractPath.Length - 1;
                int2 endPosition = new int2(
                    abstractPath[ index ].x ,
                    abstractPath[ index ].y );
        ...
        private bool LOSBetween2Clusters( int2 start , int2 end )
        {
            int2 dif = new int2(
            ...
            {
                int2 clusterPosition = new int2(
                    pos.x / graphClusterLength ,
                    pos.y / graphClusterLength );
                int clusterIndex = clusterPosition.x + clusterPosition.y * graphClusterLength;
